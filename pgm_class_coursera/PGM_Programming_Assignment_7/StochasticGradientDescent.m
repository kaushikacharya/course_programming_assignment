...
function thetaOpt = StochasticGradientDescent (gradFunc, theta0, maxIter)
    ...
    % https://class.coursera.org/pgm/forum/thread?thread_id=2013
    % "a function handle for a function that computes the gradient of the
    % negative log- likelihood of the data, at a particular ? and for a particular iteration number k."
    %   Posted by Matthew Saul Leifer (Student)  

    for k = 1:maxIter
        alpha_k = 0.1/(1+sqrt(k));
        rand_index = floor(10000*rand());
