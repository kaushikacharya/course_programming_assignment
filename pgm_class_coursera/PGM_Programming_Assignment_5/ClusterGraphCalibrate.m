...
for m = 1:length(edgeFromIndx),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % MESSAGES(i,j).var = setdiff(P.clusterList(i).var,P.clusterList(j).var);
    [MESSAGES(i,j).var, index_from, index_to] = intersect(P.clusterList(i).var,P.clusterList(j).var);
    MESSAGES(i,j).card = P.clusterList(i).card(index_from);
...
while (1),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    neighbours_to_be_considered = setdiff(find(P.edges(:,i)),j);
    % first get the factor product of all incoming messages to cluster i
    % except from cluster j
    factProd = MESSAGES(neighbours_to_be_considered(1),i);
    for index_i = 2:length(neighbours_to_be_considered)
        ngbr_cluster_i = neighbours_to_be_considered(index_i);
        factProd = FactorProduct(factProd,MESSAGES(ngbr_cluster_i,i));
    end
%     factProd = P.clusterList(neighbours_to_be_considered(1));
%     for index_i = 2:length(neighbours_to_be_considered)
%         ngbr_cluster_i = neighbours_to_be_considered(index_i);
%         factProd = FactorProduct(factProd,P.clusterList(ngbr_cluster_i));
%     end
    % now do factor product with cluster i belief
    factProd = FactorProduct(factProd,P.clusterList(i));
    % now marginalize to get the new message to pass from cluster i to
    % cluster j
    MESSAGES(i,j) = FactorMarginalization(factProd,setdiff(factProd.var,MESSAGES(i,j).var));
    % normalize
    MESSAGES(i,j).val = MESSAGES(i,j).val/sum(MESSAGES(i,j).val);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
