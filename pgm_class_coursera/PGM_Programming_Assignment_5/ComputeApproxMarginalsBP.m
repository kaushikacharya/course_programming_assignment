...
function M = ComputeApproxMarginalsBP(F,E)
    ...
    for i = 1:length(N),
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        cluster_i = FindPotentialWithVariable(P, i);
        
        if isempty(setdiff(P.clusterList(cluster_i).var,i))
            M(i) = P.clusterList(cluster_i);
        else
            M(i) = FactorMarginalization(P.clusterList(cluster_i),setdiff(P.clusterList(cluster_i).var,i));
        end
