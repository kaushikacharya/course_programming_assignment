...
for i = 1:size(genotypesToAlleles,1)
    allele1 = genotypesToAlleles(i,1);
    allele2 = genotypesToAlleles(i,2);
    
    % alternate way: use allelesToGenotypes
    if allele1 ~= allele2
        mult_factor = 2;
    else
        mult_factor = 1;
    ...
    genotypeFactor.val(i) = mult_factor*alleleFreqs(allele1)*alleleFreqs(allele2);
