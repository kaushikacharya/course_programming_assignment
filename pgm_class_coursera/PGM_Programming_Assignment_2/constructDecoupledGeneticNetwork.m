...
for i = 1:numPeople
    if sum(pedigree.parents(i,:)) == 0
        ...
        % parent present, assumed both parents
        geneCopyOneParentOne = pedigree.parents(i,1);
        geneCopyTwoParentOne = pedigree.parents(i,1)+numPeople;
        factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, geneCopyOneParentOne, geneCopyTwoParentOne);
    end
end

for i = numPeople+1:2*numPeople
    if sum(pedigree.parents(i-numPeople,:)) == 0
        % no parent 
        factorList(i) = childCopyGivenFreqsFactor(alleleFreqs, i);
    else
        % parent present, assumed both parents
        geneCopyOneParentTwo = pedigree.parents(i-numPeople,2);
        geneCopyTwoParentTwo = pedigree.parents(i-numPeople,2)+numPeople;
        factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, geneCopyOneParentTwo, geneCopyTwoParentTwo);
    end
end
