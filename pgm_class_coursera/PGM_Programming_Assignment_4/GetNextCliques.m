...
% Posted by John Wallerius (Student)
% on Sun 22 Apr 2012 11:48:17 AM PDT
% Read the comment for explanation of Messages

for row_i = 1:length(P.cliqueList)
    % first get the cliques to which clique row_i is connected
    col_array = find(P.edges(row_i,:));
    neighbour_source_array = find(P.edges(:,row_i));
    for col_i = col_array
        % check if row_i -> col_i message has already been passed
        if isempty(messages(row_i,col_i).var)
            % now confirm that all neighbours of row_i(except col_i) have
            % passed their messages to row_i
            neighbours_to_be_considered = setdiff(neighbour_source_array,col_i);
            flag_msg_arrived_from_neighbours = true;
            for neighbour = neighbours_to_be_considered'
                if isempty(messages(neighbour,row_i).var)
                    flag_msg_arrived_from_neighbours = false;
                    break;
                end
            end
            
            if flag_msg_arrived_from_neighbours
                i = row_i;
                j = col_i;
                break;
            end
