function a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  from_data_file = load('data/data.mat');
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  # fprintf('size(class_normalizer): %d\n', size(class_normalizer))
  
  # fprintf('size(ret.hid_to_class): %d\n', size(ret.hid_to_class))
  # fprintf('size(hid_input): %d\n', size(hid_input))
  # fprintf('size(hid_output): %d\n', size(hid_output))
  # fprintf('size(class_prob): %d\n', size(class_prob))
  # fprintf('size(data.targets): %d\n', size(data.targets))
  
  # fprintf('size(data.inputs): %d\n', size(data.inputs))
  # fprintf('size(ret.input_to_hid): %d\n', size(ret.input_to_hid))
  
  ret.hid_to_class = ret.hid_to_class + ((class_prob - data.targets) * transpose(hid_output))./size(hid_output, 2);
  
  ret.input_to_hid = ret.input_to_hid + (transpose(model.hid_to_class) * (class_prob - data.targets) .* hid_output .* (ones(size(hid_output,1), size(hid_output,2)) - hid_output) * transpose(data.inputs))./ size(data.inputs, 2);
end
