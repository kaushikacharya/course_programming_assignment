...
# Added by KA
# In (4a) link has been provided to spark 1.6.1 documentation pages
...
# Added by KA to run above example
...
# Added by KA
df.head(1)[0].asDict()['first_name']

# COMMAND ----------

# Added by KA (method #1 in above example)
from pyspark.sql.functions import udf
u_make_hash = udf(make_hash)
df2 = df.select(df['*'], u_make_hash(df['first_name'], df['last_name']).alias('hash'))
# could run df2.show() here to prove it works
df2.show()

# COMMAND ----------

# Added by KA (method #2 in above example)
df2 = (df.rdd
...
# Added by KA for trial
from pyspark.sql import functions as sql_functions
test_df = raw_data_df.select(sql_functions.split(raw_data_df.value, ',').alias('value'))
...
# Added by KA: create labeled_df using RDD
labeled_df = (test_df.rdd
                     .map(lambda row: LabeledPoint(float(row.value[0]), [float(x) for x in row.value[1:]]))
                     .toDF())                   
labeled_df.show()

# COMMAND ----------

# Added by KA: create labeled_df using dataframe
# TBD: This is throwing error
"""
from pyspark.sql import functions as sql_functions

def make_label_features(val):
  # Note: each row of dataframe has already been split
  assert instance(val,list), "val should be a list of values"
  label = float(val[0])
  features = [float(x) for x in val[1:]]
  labeled_point = LabeledPoint(label, features)
  return labeled_point

udf_make_label_features = sql_functions.udf(make_label_features)
labeled_df = test_df.select(test_df['*'], udf_make_label_features(test_df['value']))
print "type(labeled_df): ", type(labeled_df)
labeled_df.show()
"""
...
def make_label_features(val):
  ...
  print labeled_point
  # return labeled_point
  assert isinstance(labeled_point, LabeledPoint), "labeled_point should be LabeledPoint"
  return labeled_point
"""  
def parse_points(df):
    ...
    udf_make_label_features = sql_functions.udf(make_label_features)
    parsed_points_df = df.select(df['*'], udf_make_label_features(df['value']))
    return parsed_points_df
    """
    from pyspark.sql import functions as sql_functions
    split_df = df.select(sql_functions.split(df.value, ',').alias('value'))
    # print split_df.head(2)
    labeled_df = (split_df.rdd
                          .map(lambda row: LabeledPoint(row.value[0], [x for x in row.value[1:]]))
                          .toDF())
    return labeled_df

parsed_points_df = parse_points(raw_data_df)
# print 'type(parsed_points_df): ', type(parsed_points_df)
# print 'columns(parsed_points_df): ', parsed_points_df.columns
# parsed_points_df.show()
first_point = parsed_points_df.first()
# print 'type(first_point): ', type(first_point)
# print first_point
first_point_features = first_point.asDict()['features']
first_point_label = first_point.asDict()['label']
...
# resource:
# https://piazza.com/class/iqfbu516yuj5t3?cid=334
# (The pyspark.sql.functions.split expects a column as an input (either directly or by name), not a string.)

# http://stackoverflow.com/questions/35067467/how-can-i-get-from-pyspark-sql-types-row-all-the-columns-attributes-name

# COMMAND ----------


# Added by KA
# parsed_points_df.columns
# parsed_points_df.printSchema()
parsed_points_df.schema

# COMMAND ----------

# Added by KA
print type(first_point_features)
print type(first_point_features[0])
print type(first_point_label)
...
"""
min_year = content_stats.head(1)[0][0]
max_year = content_stats.head(1)[0][1]
"""
...
# TODO: Replace <FILL IN> with appropriate code
# parsed_data_df = parsed_points_df.select(parsed_points_df['features'], (parsed_points_df['label']-min_year).cast("int")).withColumnRenamed("CAST((label - {0}) AS INT)".format(min_year), "label")
# parsed_data_df = parsed_points_df.select(parsed_points_df['features'], (parsed_points_df['label']-min_year)).withColumnRenamed("(label - {0})".format(min_year), "label")
...
# Added by KA
# spark.catalog.clearCache() # applicable in spark 2.0+ (?? check exact version)
sqlContext.clearCache()
# https://stackoverflow.com/questions/20517723/how-to-make-shark-spark-clear-the-cache
...
# https://piazza.com/class/iqfbu516yuj5t3?cid=518 (Issue with 1e) Training, validation, and test sets & (2a) Average label)
# https://piazza.com/class/iqfbu516yuj5t3?cid=174 

# COMMAND ----------

# Added by KA to clear cache
"""
parsed_train_data_df.unpersist()
parsed_val_data_df.unpersist()
parsed_test_data_df.unpersist()
"""
...
def make_prediction_label(average_train_year, row):
  return (average_train_year, row[1])

# preds_and_labels_train = parsed_train_data_df.rdd.map(lambda row: make_prediction_label(average_train_year, row))
...
# https://piazza.com/class/iqfbu516yuj5t3?cid=1197 (instructor's answer on prediction value)
...
def linreg_gradient_descent(train_data, num_iters):
    ...
    for i in range(num_iters):
        ...
        preds_and_labels_train = train_data.map(lambda lp: get_labeled_prediction(w, lp))  # <FILL IN>
        preds_and_labels_train_df = sqlContext.createDataFrame(preds_and_labels_train, ["prediction", "label"])
        ...
        gradient = train_data.map(lambda lp: gradient_summand(w, lp)).sum() # <FILL IN>
        # print "type(gradient): ", type(gradient) # added by KA
        # print "gradient: ", gradient # added by KA

        # Update the weights
        alpha_i = alpha / (n * np.sqrt(i+1))
        # print "type(alpha_i): ", type(alpha_i) # added by KA
...
# https://piazza.com/class/iqfbu516yuj5t3?cid=1212 (In 3b) have you cast each member of the tuple you return to float?)

# COMMAND ----------

# Added by KA
example_data.toDF().show()
...
# Added by KA
parsed_val_data_df.show()

# COMMAND ----------

# TODO: Replace <FILL IN> with appropriate code
num_iters = 50
weights_LR0, error_train_LR0 = linreg_gradient_descent(parsed_train_data_df.rdd, num_iters) # <FILL IN>

preds_and_labels = (parsed_val_data_df
                      .rdd
                      .map(lambda lp: get_labeled_prediction(weights_LR0, lp))) # <FILL IN>
preds_and_labels_df = sqlContext.createDataFrame(preds_and_labels, ["prediction", "label"])
...
# TODO: Replace <FILL IN> with appropriate code
lin_reg = LinearRegression(regParam=reg, elasticNetParam=alpha, maxIter=num_iters, fitIntercept=use_intercept) # <FILL IN>
...
coeffs_LR1 = first_model.coefficients # <FILL IN>
intercept_LR1 = first_model.intercept # <FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
sample_prediction = first_model.transform(dataset=parsed_train_data_df) # <FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
val_pred_df = first_model.transform(dataset=parsed_val_data_df).select('prediction', 'label') # <FILL IN>
rmse_val_LR1 = calc_RMSE(val_pred_df) # <FILL IN>
...
for reg in list([1e-10, 1e-5, 1.0]): # <FILL IN>
...
for elem in itertools.product(range(4), repeat=2):
  print "{0} : {1}".format(elem, elem[0]*elem[1])

# COMMAND ----------

# Added by KA
test_lp = LabeledPoint(0.0, [1,2,3])
print "original features: ", test_lp.features
derived_features = []
for feature_pair in itertools.product(test_lp.features, repeat=2):
  derived_features.append(feature_pair[0]*feature_pair[1])
  
print "derived_features: ", derived_features
test_lp.features = np.hstack((test_lp.features,derived_features))
print "final lp features: ", test_lp.features

# COMMAND ----------

# Added by KA
print test_lp.features[2]
...
# Added by KA
parsed_train_data_df.rdd.first()['features']

# COMMAND ----------

# Added by KA
"""
train_data_interact_df = parsed_train_data_df.rdd.map(lambda row: LabeledPoint(label=row['label'], features=row['features'])).toDF()
train_data_interact_df.show()
"""
...
def two_way_interactions(lp):
    ...
    # <FILL IN>
    derived_features = []
    for feature_pair in itertools.product(lp.features, repeat=2):
      derived_features.append(feature_pair[0]*feature_pair[1])
      
    # Now append the 2-way interaction features
    updated_features = np.hstack((lp.features, derived_features))
    lp_updated = LabeledPoint(label=lp.label, features=updated_features)
...
train_data_interact_df = parsed_train_data_df.rdd.map(lambda row: two_way_interactions(LabeledPoint(label=row.label, features=row.features))).toDF() # <FILL IN>
val_data_interact_df = parsed_val_data_df.rdd.map(lambda row: two_way_interactions(LabeledPoint(label=row.label, features=row.features))).toDF() # <FILL IN>
test_data_interact_df = parsed_test_data_df.rdd.map(lambda row: two_way_interactions(LabeledPoint(label=row.label, features=row.features))).toDF() # <FILL IN>

# COMMAND ----------

# Added by KA
"""
train_data_interact_df = parsed_train_data_df.rdd.map(lambda row: two_way_interactions(LabeledPoint(label=row.label, features=row.features))).toDF()
print type(train_data_interact_df)
print train_data_interact_df.show()
"""
