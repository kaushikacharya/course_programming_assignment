...
sample_ohe_dict_manual[(0, 'bear')] = 0 # <FILL IN >
sample_ohe_dict_manual[(0, 'cat')] = 1 # <FILL IN >
sample_ohe_dict_manual[(0, 'mouse')] = 2 # <FILL IN >
"""
...
a_sparse = SparseVector(4, [1,3], [3.0, 4.0]) # <FILL IN >

b_dense = np.array([0., 0., 0., 1.])
b_sparse = SparseVector(4, [(3,1.0)]) # <FILL IN >
...
# TODO: Replace <FILL IN> with appropriate code
sample_one_ohe_feat_manual = SparseVector(7, [2,3], [1.0,1.0]) # <FILL IN >
sample_two_ohe_feat_manual = SparseVector(7, [1,4,5], [1.0,1.0,1.0]) # <FILL IN >
sample_three_ohe_feat_manual = SparseVector(7, [(0,1.0), (3,1.0), (6,1.0)]) # <FILL IN >
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    for raw_feat in raw_feats:
      feature_index = ohe_dict_broadcast.value[raw_feat]
      encoding_val.append((feature_index,1.0))
    return SparseVector(num_ohe_feats, encoding_val)

# Calculate the number of features in sample_ohe_dict_manual
num_sample_ohe_feats = len(sample_ohe_dict_manual) # <FILL IN >
...
sample_one_ohe_feat = one_hot_encoding(sample_one, sample_ohe_dict_manual_broadcast, num_sample_ohe_feats) # <FILL IN >
...
def ohe_udf_generator(ohe_dict_broadcast):
    ...
    """
    length = len(ohe_dict_broadcast.value) # <FILL IN>
...
eDF = sqlContext.createDataFrame([Row(a=1, intlist=[1,2,3], mapfield={"a": "b", "c":"d"})])
eDF.select(explode(eDF.mapfield).alias("key","value")).show()

# COMMAND ----------

# Added by KA
sample_data_df.select(explode(sample_data_df.features)).distinct().show()
...
def download_criteo(url):
  ...
  # Added following 2 lines by KA
  print("list of files", tf.getnames())
  # print(tf.getnames())
...
# criteo_url = '<FILL IN>'
# criteo_url = 'https://s3-eu-west-1.amazonaws.com/criteo-labs/dac.tar.gz' # source: http://labs.criteo.com/2014/02/kaggle-display-advertising-challenge-dataset/
criteo_url = 'http://labs.criteo.com/wp-content/uploads/2015/04/dac_sample.tar.gz' # source: http://datascience-enthusiast.com/Python/ctr_spark.html
...
def parse_point(point):
    ...
    """
    # <FILL IN>
    tokens = point.strip().split(',')
    output = []
    for i in range(1,len(tokens)):
        output.append((i-1,tokens[i]))
    return output
...
from pyspark.sql.types import DoubleType # Added by KA
...
parsed_train_df = parse_raw_df(raw_train_df) # <FILL IN>
# https://piazza.com/class/iqfbu516yuj5t3?cid=1254 (You should be passing raw_train_df instead of raw_df to your call to parse_raw_df.)
# https://piazza.com/class/iqfbu516yuj5t3?cid=1127 
...
# Added by KA
parsed_train_df.show(truncate=False)
...
# Added by KA
test_df = sqlContext.createDataFrame([('1abs12df',)],['s',])
test_df.show()
# test_df.select(split(test_df.s,'[0-9]+').getItem(1).alias('s')).collect()
test_df.select(split(test_df.s,'^[\s]').getItem(0).alias('s')).collect()
...
# TBD(KA): This works but is this how its expected in exercise?
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    encoding_val = []
    for raw_feat in raw_feats:
      if raw_feat in ohe_dict_broadcast.value:
        feature_index = ohe_dict_broadcast.value[raw_feat]
        encoding_val.append((feature_index,1.0))
...
parsed_validation_df = parse_raw_df(raw_validation_df) # Added by KA
ohe_validation_df = (parsed_validation_df
                     .select(parsed_validation_df.label, ohe_dict_missing_udf(parsed_validation_df.feature).alias('features'))
                     .cache()
                     )
...
lr_model_basic = lr.fit(ohe_train_df) # <FILL IN>
...
def add_log_loss(df):
    ...
    return df.select(df.p, df.label, when( df.label==1, when(df.p==0, -log(df.p+epsilon)).when(df.p==1, -log(df.p-epsilon)).otherwise(-log(df.p)) ).otherwise( when(df.p==0,-log(1-df.p-epsilon)).when(df.p==1,-log(1-df.p+epsilon)).otherwise(-log(1-df.p)) ).alias('log_loss') )

# add_log_loss(example_log_loss_df).show() # commented by KA
...
# Added by KA
ohe_train_df.show(2, truncate=False)
...
from pyspark.sql.functions import mean # Added by KA
# class_one_frac_train = (<FILL IN>)
class_one_frac_train = ohe_train_df.select(mean('label')).first()[0]
print 'Training class one fraction = {0:.3f}'.format(class_one_frac_train)

# log_loss_tr_base = (<FILL IN>)
# log_loss_tr_base = add_log_loss(ohe_train_df.select(lit(class_one_frac_train).alias('p'),ohe_train_df.label,ohe_train_df.features)).select(mean('log_loss')).first()[0]
log_loss_tr_base = add_log_loss(ohe_train_df.withColumn('p',lit(class_one_frac_train))).select(mean('log_loss')).first()[0]
...
# Added by KA
type(class_one_frac_train)
# ohe_train_df.select(mean('label')).show()
ohe_train_df.select(mean('label')).first()[0]

# COMMAND ----------

# Added by KA
# add_log_loss(ohe_train_df.select(lit(class_one_frac_train).alias('p'),ohe_train_df.label,ohe_train_df.features)).show(2)
add_log_loss(ohe_train_df.select(lit(class_one_frac_train).alias('p'),ohe_train_df.label,ohe_train_df.features).where(ohe_train_df.label==1)).show(2)

# COMMAND ----------

# Added by KA
add_log_loss(ohe_train_df.select(lit(class_one_frac_train).alias('p'),ohe_train_df.label,ohe_train_df.features)).select(mean('log_loss')).show()
...
def add_probability(df, model):
    ...
    def get_p(features):
        ...
        # print "type(features): ", type(features) # Added by KA
        ...
        # <FILL IN>
        return 1.0/(1.0 + exp(-1*raw_prediction))
...
def evaluate_results(df, model, baseline=None):
    ...
    # log_loss = <FILL IN>
    log_loss = with_log_loss_df.select(mean('log_loss')).first()[0]
    return log_loss
...
log_loss_val_base = evaluate_results(ohe_validation_df, None, ohe_train_df.select(mean('label')).first()[0])
# log_loss_val_l_r0 = <FILL IN>
log_loss_val_l_r0 = evaluate_results(ohe_validation_df, lr_model_basic) # Note: lr_model_basic = lr.fit(ohe_train_df)
...
# Added by KA
# print log_loss_val_base
print "frac(train): {0} :: frac(validation): {1}".format(ohe_train_df.select(mean('label')).first()[0], ohe_validation_df.select(mean('label')).first()[0])
...
hash_train_df = add_hashed_features(parse_raw_df(raw_train_df)) # <FILL IN>
hash_validation_df = add_hashed_features(parse_raw_df(raw_validation_df)) # <FILL IN>
hash_test_df = add_hashed_features(parse_raw_df(raw_test_df)) # <FILL IN>
...
def get_sparsity(df):
    ...
    """
    # return (<FILL IN>)
    return df.select(mean(feature_sparsity_udf(df.features))).first()[0]
...
class_one_frac_test = hash_test_df.select(mean('label')).first()[0]
